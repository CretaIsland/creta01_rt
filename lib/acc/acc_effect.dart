...
class ACCEffect extends ACC {
  ACCEffect({
    required PageModel? page,
    required BaseWidget accChild,
    required double idx,
  }) : super(page: page, accChild: accChild, idx: idx.toInt());

  @override
  void initSizeAndPosition() {
    super.initSizeAndPosition();
    accModel.containerOffset.init(const Offset(0, 0));
    if (accModel.containerSize.value.width == page!.width.value.toDouble() &&
        accModel.containerSize.value.height == page!.height.value.toDouble()) {
  ...
  Future<void> next({bool pause = false}) async {
    ...
    logHolder.log('ACCEffect.next()', level: 6);
    if (player != null) {
      player.next();
    } else {
      logHolder.log('Current EffectWidget.is null', level: 6);
    }
  }
  ...
  Future<void> prev({bool pause = false}) async {
    AbsPlayWidget? player = await accChild.playManager.getCurrent();
    logHolder.log('ACCEffect.prev()', level: 6);
    if (player != null) {
      player.prev();
    } else {
      logHolder.log('Current EffectWidget.is null', level: 6);
