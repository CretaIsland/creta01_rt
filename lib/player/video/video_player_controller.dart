...
class VideoPlayerController extends ValueNotifier<VideoPlayerValue> {
  ...
  //skpark add
  void Function(VideoEvent event)? onAfterVideoEvent;
    ...
    void eventListener(VideoEvent event) {
      ...
      switch (event.eventType) {
        ...
        case VideoEventType.completed:
          ...
          //skpark
          // complete 가 연달아서 계속 오기 때문에, 0.6 초 미만차이로 오는 것은 버린다.         DateTime now = DateTime.now();
          DateTime now = DateTime.now();
          if (completeTime == null) {
            //최초시기
            completeTime = now;
            logHolder.log('duration=init');
            onCompleteEvent(event);
          } else {
            Duration duration = now.difference(completeTime!);
            completeTime = now;
            logHolder.log('duration=$duration', level: 7);
  ...
  void onCompleteEvent(VideoEvent event) {
    pause().then((void pauseResult) {
      seekTo(value.duration);
      //skpark add
      //_completeCounter++;
      //logHolder.log('completeCounter=$_completeCounter');
      //if (_completeCounter == 2) {
      if (onAfterVideoEvent != null) {
        //_completeCounter = 0;
        onAfterVideoEvent!(event);
      }
      //}
    });
  }
  ...
  Future<void> dispose() async {
    logHolder.log('controller disposed $dataSource', level: 1);
  ...
  Future<void> _applyLooping() async {
    if (_isDisposedOrNotInitialized) {
      logHolder.log('_isDisposedOrNotInitialized', level: 1);
      return;
    }
  ...
  //   // TOD: This would be more efficient as a binary search.
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    switch (state) {
      case AppLifecycleState.paused:
        logHolder.log('AppLifecycleState.paused', level: 1);
        _wasPlayingBeforePause = _controller.value.isPlaying;
        _controller.pause();
        break;
      case AppLifecycleState.resumed:
        if (_wasPlayingBeforePause) {
          logHolder.log('AppLifecycleState.played', level: 1);
          _controller.play();
        }
        break;
      default:
...
class VideoProgressIndicatorState extends State<VideoProgressIndicator> {
  ...
  @override
  void dispose() {
    logHolder.log('VideoProgressIndicatorState dispose', level: 5);
    super.dispose();
  }

  @override
  void deactivate() {
    logHolder.log('VideoProgressIndicatorState deactivate', level: 5);
